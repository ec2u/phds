...
Identify and report all "Clashes" between the document and the policy provided below. A Clash is a substantive
discrepancy where the content within the <document> tags lessens, contradicts, or improperly alters a requirement set by
the content within the <policy> tags. Your entire output must be in the specified {{target_language}}.
...
2. Super-compliance (Stricter Standards): The document meets and exceeds the minimum requirements of the policy. If the
   policy sets a floor, anything at or above that floor is compliant. Example: The policy requires students to be
...
3. Governing Policy Excerpt: The specific, relevant clause or sentence from the <policy>. This MUST be translated into
   {{target_language}}: do not use any other language.
4. Conflicting Document Excerpt: The specific, conflicting clause or sentence from the <document>. This MUST be
...
Very Important:
If there are already KNOWN ISSUES, return an empty list—unless there are extremely critical contradictions between the
document and the policy, which are unlikely to occur. If no KNOWN ISSUES exist, then check for and report all potential
clashes.

You MUST be certain that every clash has been reported. It is essential to ensure that a second analysis would not
reveal any new or different clashes.
