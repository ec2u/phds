...
3. Governing Policy Excerpt: The specific, relevant clause or sentence from the `{{policy_name}}` (translated into
   `{{target_language}}`).
4. Conflicting Document Excerpt: The specific, conflicting clause or sentence from the `{{document_name}}` (translated
...
**Important**: if no significant clashes other then the KNOWN ISSUES are detected, feel free to return an empty list
with the given structure. Never invent irrelevant or inexistent inconsistencies.
