...
The current transport is the Forge bridge (`invoke()`). The API is designed so that it can be migrated to REST/HTTP
with minimal changes. The table below shows both the resolver handlers and their conceptual REST equivalents.
...
Subsequent reads return the error to the client. Clearing the `Trace` resets the key to empty, allowing the next read
to trigger a fresh job.
